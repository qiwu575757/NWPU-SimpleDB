...
public class BTreeFile implements DbFile {
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		int movTuples = total / 2;
		// get leafpage's tuple reverse iterator for moving
		...
		// if the old page have right page, need to update pointer
		BTreePageId rightSiblingID = page.getRightSiblingId();
		BTreeLeafPage oldRightPage = (rightSiblingID==null) ? null
		...
		return field.compare(Op.GREATER_THAN, mid) ? rightPage : page;
	}
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int moveEntries = total / 2;
		Iterator<BTreeEntry> it = page.reverseIterator();
		// copy entry to new page
		while ( moveEntries > 0 ) {
			BTreeEntry e = it.next();
			// use deleteKeyAndRightChild not deleteKeyAndLeftChild for reverse iterate
			page.deleteKeyAndRightChild(e);
			rightPage.insertEntry(e);
			moveEntries--;
		}
		// push out the midentry
		...
		// get parent node from midentry and insert midentry to it
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int avgTuples = (curTuples+siblingTuples) / 2;
		if (isRightSibling) {
			// steal from right sibling
			Iterator<Tuple> it = sibling.iterator();
			while (it.hasNext() && curTuples < avgTuples) {
				Tuple t = it.next();
				sibling.deleteTuple(t);
				page.insertTuple(t);
				curTuples++;
			}
			Tuple midTuple = sibling.iterator().next();
			entry.setKey(midTuple.getField(parent.keyField));
			...
			Tuple midTuple = page.iterator().next();
			entry.setKey(midTuple.getField(parent.keyField));
			parent.updateEntry(entry);
		}
	}
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int avgEntries = (curEntries + leftSiblingEntries) / 2;
		Iterator<BTreeEntry> it = leftSibling.reverseIterator();

		// 先处理parententry and leftsibling 倒数第一个节点
		BTreeEntry right = page.iterator().next();
		BTreeEntry left = it.next();
		leftSibling.deleteKeyAndLeftChild(left);
		...
		while (it.hasNext() && curEntries < avgEntries) {
			BTreeEntry e = it.next();
