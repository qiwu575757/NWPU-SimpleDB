            ...
            synchronized(this) {
                ...
                while (true) {
                    try {
                        ...
                        switch (type) {
                            case UPDATE_RECORD:
                                // update records 包括 before image and after image
                                // 如果before image pid == txid 表明是要修改的日志
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                    ...
                    try {
                        ...
                        switch(type) {
                            ...
                            case COMMIT_RECORD:
                                ...
                                break;
                            case CHECKPOINT_RECORD:
                                // CHEKCPOINT RECORDS 包括了 一个整数表明当前监测点的事务数
                                // 还有一个长整数表明 事务id，一个长整数表示record offset
                ...
                // 处理未提交事务，直接写 beforeimage
                for (long txid: beforePages.keySet()) {
                    // 若已提交事务集合中未包含该事务,需要将该事务所修改的页面重写
